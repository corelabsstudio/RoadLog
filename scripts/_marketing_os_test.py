...
def main() -> None:
    ...
    try:
        ...
        check(marketing_os.run_due(web, before) == [], "예약 시간 전 자동 실행 금지")
        ...
        check(sum(len(result) for result in concurrent) == 1, "동시 작업자 간 하루 한 번만 예약 실행")
        failing = marketing_os.run_due(temp / "missing-web", datetime(2026, 9, 24, 11, 1, tzinfo=ZoneInfo("Asia/Seoul")))
        check(len(failing) == 1 and failing[0]["status"] == "FAILED", "정본 오류 시 예약 작업 실패 상태")

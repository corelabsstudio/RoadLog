...
CARD_SCHEMA = {
    ...
    "properties": {
        ...
        "tale": {"type": "string",
                 "description": "두 문장. 퍼뜨리고 싶은 두 줄로 닫는다. "
                                "🛑 일흔 자 안쪽 — 카드에 세 줄까지만 들어간다"},
    },
    ...
    "properties": {
        ...
        "summary": {"type": "string",
                    "description": "보자마자 「이거 완전 나잖아」 하고 공감해서 스레드에 "
                                   ...
                                   "🛑 **일흔 자 안쪽** — 카드에 세 줄까지만 들어간다. 넘으면 문장이 통째로 빠진다"},
    },
...
[규칙]
- 두 문장. 합쳐서 **45자 안쪽.** 넘으면 카드에서 잘린다.
  🛑 짧을수록 좋다. 한 문장으로 끝나면 그게 가장 좋다.
...
- 🛑 **물음에 답하는 자리를 찾아 읽어라.** 결과지가 자리마다 [제목]으로 나뉘어 들어온다.
  **첫 자리부터 쓰지 마라** — 첫 자리는 대개 성격 이야기다.
  「몇 살에」를 물으면 **때를 말하는 자리**를, 「누가」를 물으면 **사람을 말하는 자리**를
  읽는다. 때·나이를 물었는데 성격으로 답하면 그건 틀린 답이다.
...
# 🛑 요약을 만드는 방식이 바뀌면 올린다. 서버가 이 값보다 낮게 저장된 요약을
#    **다시 만든다** — 안 그러면 옛 방식으로 쓴 글이 카드에 계속 나간다.
SUM_VER = 2


def _summary_src(blocks: list[dict[str, Any]]) -> list[str]:
    """카드 문구를 쓸 재료. **자리를 통째로 준다.**

    🛑 **앞 둘 + 마지막만 주지 않는다** (2026-09-11 온해님 「질문과 답이 안 맞아」).
       「내 인생 전체 스포」는 자리가 열여섯인데, 물음(「최고 리즈 시절은 몇 살에」)의
       답은 여덟 번째 자리 「인생에서 가장 빛나는 때」에 있었다. 그 자리가 재료에
       없으니 LLM 은 물음을 알고도 답할 수가 없었고, 첫 자리(성격)로 답을 썼다.
       **물음만 주고 답이 든 대목은 안 준 것**이 진짜 원인이다 — 그전 두 번은
       프롬프트만 고쳤고, 그래서 같은 일이 되풀이됐다.
    🛑 **제목을 같이 준다.** 어느 자리에서 온 글인지 알아야 물음에 맞는 대목을 고른다.
    🛑 **마지막 자리는 넉넉히 준다.** 대개 그 상품의 종합이다.
    """
    n = len(blocks)
    # 전체를 9,000자 안쪽으로 — 마흔다섯 자리짜리 대점도 여기 들어온다
    room = 900 if n <= 3 else max(200, min(600, 9000 // n))
    src = []
    for i, b in enumerate(blocks):
        ...
        head = ("[%s]" % ttl + chr(10)) if ttl else ""
        src.append(head + t[:(900 if i == n - 1 else room)])
    return src
...
def summarize(blocks: list[dict[str, Any]], *, question: str = "",
    ...
    src = _summary_src(blocks)
    if not src:
        return ""
    user = ("[결과지 — 자리마다 제목이 붙어 있다. 물음에 답하는 자리를 골라 읽어라]"
            + chr(10) + (chr(10) * 2).join(src))
    ...
    for _ in range(2):
        ...
        if txt:
            break
    # 🛑 **70자다** (2026-09-11 온해님 「글자수를 줄이는 게 낫겠다」).
    #    90자는 카드 세 줄을 넘겨서 마지막 문장이 통째로 잘려 나갔다.
    return _cut_sentence(txt, 70)
...
{"name": "…", "line": "…", "tale": "…"}
  ...
  tale  두 문장. 그 전생이 지금 나에게 무엇을 남겼는지로 닫는다
        🛑 **일흔 자 안쪽.** 카드에 세 줄까지만 들어간다 — 넘으면 문장이 통째로
        빠진다. 쓰고 나서 세어 보고 넘으면 줄여라. 한 문장으로 끝나면 그게 낫다
...
# 🛑 `tale` 은 200자였다 (2026-09-11 온해님 「글자수를 줄이는 게 낫겠다」).
#    카드에 세 줄까지만 들어가는데 200자를 담아서 마지막 문장이 잘려 나갔다.
_CARD_MAX = {"name": 24, "line": 40, "tale": 74, "badge": 24,
...
def write_card(kind: str, facts: dict[str, str], *,
    ...
    for k in _CARD_KEYS:
        ...
        if k in ("name", "line", "job", "habit", "karma"):
            v = v.rstrip(" .。")
        if v:
            # 🛑 **`tale` 은 문장 중간에서 끊지 않는다** (2026-09-11 온해님이 잡으심).
            #    글자 수로만 자르니 카드 마지막 줄이 「…왕 노릇 했다간」에서 끊겼다.
            #    카드는 스크린샷으로 퍼지는 물건이라 그 조각이 그대로 돌아다닌다.
            #    이름·한 줄처럼 짧은 칸은 애초에 한 문장이라 그냥 자른다.
            out[k] = _cut_sentence(v, _CARD_MAX[k]) if k == "tale" else v[:_CARD_MAX[k]]
    if fixed:

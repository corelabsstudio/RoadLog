...
def _empty_profile() -> dict[str, Any]:
    return {
        ...
        "form_layout": "",
        "form_signature": "",
        # 회사 서식에 주유 확인·금액 칸이 있는지 (없으면 생성 시 주유 미기재)
        "form_has_fuel": False,
...
def list_style_status(email: str) -> dict[str, Any]:
    ...
    return {
        ...
        "form_has_fuel": bool(profile.get("form_has_fuel")),
...
form_has_fuel 은 샘플 양식에 주유·유류·연료 확인/금액/주유량 칸이 실제로 있을 때만 true.
없으면 반드시 false. 추측으로 true 하지 마세요.
"""


_FUEL_FIELD_RE = re.compile(
    r"주유|유류|연료|급유|주유량|주유금액|주유\s*비|유류비|연료비|리터|ℓ|\bL\b|fuel|refuel|gas\s*station",
    re.IGNORECASE,
)


def detect_form_has_fuel(
    profile: dict[str, Any] | None = None,
    sample_texts: list[tuple[str, str]] | list[str] | None = None,
    analysis: dict[str, Any] | None = None,
) -> bool:
    """
    회사 서식에 주유 관련 칸이 있는지 판별.
    없으면 일지 생성 시 주유 정보를 넣지 않는다.
    """
    if analysis and analysis.get("form_has_fuel") is True:
        return True
    if analysis and analysis.get("form_has_fuel") is False:
        # AI가 false 여도 샘플 텍스트에 명확히 있으면 보정
        pass
    ...
    if analysis:
        for key in ("form_header_fields", "form_columns", "form_layout", "form_title"):
            val = analysis.get(key)
            if isinstance(val, list):
                chunks.append(" ".join(str(x) for x in val))
            elif val:
                chunks.append(str(val))
        if "form_has_fuel" in analysis and analysis.get("form_has_fuel") is True:
            return True

    if sample_texts:
        for item in sample_texts:
            if isinstance(item, tuple):
                chunks.append(item[1] if len(item) > 1 else str(item[0]))
            else:
    ...
    if not blob.strip():
        return False
    return bool(_FUEL_FIELD_RE.search(blob))


def profile_has_fuel_field(email: str | None) -> bool:
    """생성 파이프라인용 — 서식에 주유 칸이 없으면 False."""
    if not email:
        return False
    profile = load_profile(email)
    if "form_has_fuel" in profile:
        return bool(profile.get("form_has_fuel"))
    # 구 프로필 호환: 학습 필드·샘플에서 재탐지
    try:
        texts = _load_sample_texts(email, profile) if profile.get("samples") else []
    except Exception:
        texts = []
    return detect_form_has_fuel(profile=profile, sample_texts=texts)
    ...
    profile.update(
        {
            ...
            "form_has_fuel": detect_form_has_fuel(
                profile=None, sample_texts=texts, analysis=analysis
            ),
            "excerpts": excerpts,
            "engine": engine,
...
def _analyze_fallback(texts: list[tuple[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "form_has_fuel": detect_form_has_fuel(sample_texts=texts),
    }
...
- 주유 칸: {"있음 — 사용자가 입력한 주유만 반영" if profile.get("form_has_fuel") else "없음 — fuel_refueled/fuel_amount_krw/fuel_liters 를 넣지 말고, 주유 내용을 요약·구간에도 쓰지 말 것"}

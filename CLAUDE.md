...
- 관리자 `AI 초안 1건 만들기` 버튼에만 중복 브라우저 확인창을 제거했다. 수동 버튼, 별도 유료 API 허용 스위치, 원자적 예산·호출 수 제한, 외부 게시 건별 승인 정책은 유지한다. `web/admin/index.html`은 `roadlog-saju/public/admin/index.html`에서 복사한 운영 사본이다. 배포 및 실제 AI 시험 결과는 별도 후속 기록으로 확인한다.
